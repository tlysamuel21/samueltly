...
class Mesh(object):
    ...
    def adjacency(self, dim1, dim2):
        ...
        if dim2 == dim1:
            return np.arange(self.entity_counts[dim1]).reshape((-1, 1))
        if dim2 < 0:
    ...
    def jacobian(self, c):
        ...
        """
        # Using the geometry of the reference element, we calculate Jacobian
        # matrix directly by subtracting the vertex coordinates. This's a fast
        # method, and while we could use a more general approach by saving the
        # gradient tabulation as an attribute (which would be more flexible),
        # this method is much more efficient for simple reference elements.
        coords = self.vertex_coords[self.cell_vertices[c, :], :]
        return (coords[1:] - coords[0]).T
